...
local function check_enable_checkbox()
    ...
    if cb then
        local new_state = cb:get_state() or true -- ✅ Force true if nil
...
function menu.init_menu()
    ...
    local state = menu.elements.enable_checkbox:get_state()
    _G.paladin_state = {
        enabled = state ~= false  -- ✅ Default ON unless explicitly false
    }
